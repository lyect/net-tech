...
public class ServerRunnable implements Runnable {
	...
	public void run() {
		...
		try {
			...
			startServer();
			log(Level.INFO, "Server started successfully! Server is listening on port " + Integer.toString(port) + ".");
			System.out.println("Server started successfully! Server is listening on port " + Integer.toString(port) + ".");
		} catch (IOException ioe) {
			log(Level.SEVERE, "Failed to start server on port " + Integer.toString(port) + ".");
			log(Level.SEVERE, ioe.getMessage());
			System.out.println("Failed to start server on port " + Integer.toString(port) + ".");
			ioe.printStackTrace();
			return;
		}
